export function createRoutePool(nodes) {
  ...
  return { nodes, choose, setAvailability(host, available) { for (const node of nodes) if (node.host === host) node.available = available; }, async query(sql, values) { return choose().query(sql, values); }, async execute(sql, values) { return choose().execute(sql, values); }, async health() { const results = []; for (const node of nodes) { try { results.push(await node.health()); } catch (error) { results.push({ ok: false, host: node.host, port: node.port, error: error.message }); } } return results; }, async close() { await Promise.all(nodes.map((node) => node.close())); } };
}
