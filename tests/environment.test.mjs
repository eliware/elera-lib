...
test('maps an optional Unix socket from the environment to the primary pool', async () => {
  ...
  expect(createPool).toHaveBeenCalledWith(expect.objectContaining({ socketPath: '/run/mysqld/mysqld.sock', host: 'localhost' }));
  await client.close();
});
...
test('maps explicit primary and balanced environment routes without a socket', async () => {
  ...
  expect(createPool.mock.calls[0][0].socketPath).toBeUndefined();
  await client.close();
});
