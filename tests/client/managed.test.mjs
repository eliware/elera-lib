import { createManagedDb } from '../../src/client/managed.mjs';
...
test('creates a managed client from endpoint and token only', async () => {
  ...
  const client = await createManagedDb({ endpoint: 'http://vip:8080', token: 'token', fetchImpl, WebSocketImpl: FakeWebSocket, mysqlLib: { createPool: () => ({ query: async () => [[]], execute: async () => [[]], getConnection: async () => ({}), end: async () => {} }) } });
