...
import { validateBundlePorts } from '../../src/bundle/ports.mjs';
...
test('validates service ports', () => {
  ...
  expect(() => validateBundle({ ...valid, ports: { sql: 3306, http: 8080, ' ws ': 9000 } })).toThrow('name');
  expect(() => validateBundlePorts({ get sql() { return 3306; }, http: 8080 })).toThrow('data properties');
});
